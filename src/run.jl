...
if Sys.islinux() || Sys.isapple()
    ...
    global defaultPage = "file:///src/SamplePages/test.sml"
    global PATH = pwd() * "/src/SamplePages/"
elseif Sys.iswindows() # Do something windows-y here... needs tested by someone with a MS machine
    if basename(pwd()) != "NaquadahBrowser" # Make sure we're in the right dir
        cd("Projects\\NaquadahBrowser")
    end
    global defaultPage = "file:///src\\SamplePages\\test.sml"
    global PATH = pwd() * "src\\SamplePages\\"
end

using Reexport
@reexport using NaquadahBrowser
